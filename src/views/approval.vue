<template>
  <div v-if="isAuth(['ROOT', 'WORKFLOW:APPROVAL', 'FILE:ARCHIVE'])">
    ...
    >
      ...
      >
        <template #default="scope">
          <span v-if="scope.row.status != '已结束'" style="color: orange"
            >审批中</span
          >
          <span
            v-if="scope.row.status == '已结束' && scope.row.result == '同意'"
            style="color: #17b3a3"
          >
            已同意
          </span>
          <span
            v-if="scope.row.status == '已结束' && scope.row.result == '不同意'"
            style="color: #f56c6c"
          >
            已拒绝
          </span>
    ...
    expand: function (row, expandedRows) {
      ...
      if (expandedRows.length > 0) {
        ...
        if (row) {
          ...
          that.$http(
            "approval/searchApprovalContent",
            "POST",
            data,
            false,
            function (resp) {

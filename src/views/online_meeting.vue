  ...
  <div>
    ...
    >
      <el-table-column
        ...
        type="expand"
      >
        <template #default="scope"> 会议内容：{{ scope.row.desc }} </template>
      </el-table-column>
      <el-table-column
        type="index"
        header-align="center"
...
export default {
  ...
  methods: {
    loadDataList: function () {
      ...
      that.$http(
        "meeting/searchOnlineMeetingByPage",
        "POST",
        data,
        true,
        function (resp) {
          ...
          for (let one of page.list) {
            ...
            let minute = dayjs(new Date()).diff(
              dayjs(`${one.date} ${one.start}`),
              "minute"
            );

            if (
              one.mine == "true" &&
              ((minute >= -10 && minute <= 0 && one.status == "未开始") ||
                one.status == "进行中")
            ) {

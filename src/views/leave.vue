<template>
  <div>
    <el-form :inline="true" :model="dataForm" :rules="dataRule" ref="dataForm">
      ...
      <el-form-item>
        <el-button @click="searchHandle()" type="primary" size="medium"
          >查询</el-button
        >
        <el-button type="danger" size="medium" @click="addHandle()"
          >我要请假</el-button
        >
      </el-form-item>
    </el-form>
    ...
    >
      <el-table-column
        ...
        type="expand"
      >
        <template #default="scope"> 请假原因：{{ scope.row.reason }} </template>
      </el-table-column>
      <el-table-column
        type="index"
        header-align="center"
      ...
      >
        <template #default="scope">
          <span v-if="scope.row.status == '请假中'" style="color: orange">{{
            scope.row.status
          }}</span>
          <span v-if="scope.row.status == '已同意'" style="color: #17b3a3">{{
            scope.row.status
          }}</span>
          <span v-if="scope.row.status == '不同意'" style="color: #f56c6c">{{
            scope.row.status
          }}</span>
        </template>
      </el-table-column>
  ...
  methods: {
    ...
    loadDataList: function () {
      ...
      if (that.dataForm.date != null && that.dataForm.date != "") {
        data.data = dayjs(that.dataForm.date).format("YYYY-MM-DD");
      }

      that.$http(
        "leave/searchLeaveByPage",
        "POST",
        data,
        true,
        function (resp) {

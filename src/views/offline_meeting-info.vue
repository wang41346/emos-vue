    ...
    <div>
      ...
      <el-row class="info member" v-if="['待审批', '未开始'].includes(status)">
        ...
        <el-col :span="21" class="value">
          <ul class="list">
            <li class="item" v-for="one in members" :key="one.id">
      ...
      <el-row class="info member" v-if="['进行中', '已结束'].includes(status)">
        ...
        <el-col :span="21" class="value">
          <ul class="list">
            <li class="item" v-for="one in present" :key="one.id">
              <img :src="one.photo" class="photo" />
              <span class="name">{{ one.name }}</span>
            </li>
          </ul>
      ...
      <el-row class="info member" v-if="['进行中', '已结束'].includes(status)">
        ...
        <el-col :span="21" class="value">
          <ul class="list">
            <li class="item" v-for="one in unpresent" :key="one.id">
              <img :src="one.photo" class="photo" />
              <span class="name">{{ one.name }}</span>
            </li>
          </ul>
  ...
  methods: {
    init: function (id, status) {
      ...
      that.$nextTick(() => {
        ...
        that.$http(
          "meeting/searchMeetingInfo",
          "POST",
          data,
          true,
          function (resp) {
            let map = resp.map;
            that.title = map.title;
            that.date = map.date;
            that.place = map.place;
            that.start = map.start;
            that.end = map.end;
            ...
            if (map.hasOwnProperty("members")) {
              that.members = JSON.parse(map.members);
            }

            if (map.hasOwnProperty("present")) {
              that.present = JSON.parse(map.present);
            }

            if (map.hasOwnProperty("unpresent")) {
              that.unpresent = JSON.parse(map.unpresent);
            }
          }
        );
      });
